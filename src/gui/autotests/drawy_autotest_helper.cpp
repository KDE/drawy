...
void AutoTestHelper::compareFile(const QString &repo, const QByteArray &data, const QString &name)
{
    const QString refFile = QLatin1StringView(DRAWY_GUI_DATA_DIR) + repo + name + u".ref"_s;
    const QString generatedFile = QLatin1StringView(DRAWY_GUI_BINARY_DATA_DIR) + repo + name + u"-generated.ref"_s;
    QDir().mkpath(QLatin1StringView(DRAWY_GUI_BINARY_DATA_DIR) + repo + name);
...
QJsonDocument AutoTestHelper::loadJsonDocument(const QString &originalJsonFile)
{
...
QJsonObject AutoTestHelper::loadJsonObject(const QString &originalJsonFile)
{
    return loadJsonDocument(originalJsonFile).object();
}

QJsonArray AutoTestHelper::loadJsonArrayObject(const QString &originalJsonFile)
{
