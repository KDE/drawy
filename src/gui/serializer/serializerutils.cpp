...
void SerializerUtils::saveInFile(const QJsonObject &obj, const QString &fileName)
{
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        qCWarning(DRAWY_GUI_LOG) << "Error: There was an error opening the save file.";
        return;
    }
    ...
    if (written != compressedData.size()) {
        qCWarning(DRAWY_GUI_LOG) << "Warning: not all bytes were written";
    }

    qCDebug(DRAWY_GUI_LOG) << "Saved to file: " << fileName;
}
