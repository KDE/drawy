...
void ItemCache::drawCached(QPainter &painter, const std::shared_ptr<Item> &item, const QRect &queryRegion, const QPointF &offset)
{
    ...
    if (m_cacheGrids.find(item) == m_cacheGrids.end()) {
        ...
        const int rows{static_cast<int>(std::ceil(transformedQueryRegion.width() / maxCellSize.width())) + 1};
        const int cols{static_cast<int>(std::ceil(transformedQueryRegion.height() / maxCellSize.height())) + 1};
